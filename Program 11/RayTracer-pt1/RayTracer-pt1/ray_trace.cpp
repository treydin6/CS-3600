...
// we will only be tracing spheres
class Sphere
{
public:
    Point3 origin;
...
class Scene // holds the whole sceen
{
public:
    Scene();
    ~Scene();
    bool Load(const char * filename); // loads model.txt
...
void Scene::CalculateRightAndUp()
{
    ...
    //u = tan(fov / 2 * pi /180)
    // r =
    double u = tan( (fov/2) * 3.14159 / 180);
    double r = ( ((float)screen_x * u )/ screen_y );
...
void Scene::CreateRay(int x, int y, Vector3 &ray)
{
    ...
    Point3 film = ww_tl + (ww_tr - ww_tl)* right_ratio + (ww_bl - ww_tl) * up_ratio;
...
// We have to finsih this code //
/////////////////////////////////
bool RayIntersectSphere(const Point3 &eye, const Vector3 & ray, const Sphere & s, float & distance, Point3 &intersection_point, Vector3 &intersection_normal ){
    double ecx, ecy, ecz, t1, t2, large, small;
    ecx = (eye.p[0] - s.origin.p[0]);
    ecy = (eye.p[1] - s.origin.p[1]);
    ecz = (eye.p[2] - s.origin.p[2]);
    
    double a, b, c;
    a = (pow(ray.v[0], 2) + pow(ray.v[1], 2) + pow(ray.v[2], 2) );
    b = ( (2 * ecx * ray.v[0]) + (2 * ecy * ray.v[1]) + (2 * ecz * ray.v[2]) );
    c = ( pow(ecx, 2) + pow(ecy, 2) + pow(ecz, 2) - (pow(s.radius, 2)) );
    ...
    if(t1 >= 0 || t2 >=0){
        ...
        intersection_point = eye + ray * large;
    ...
    return false; // if hit: true & set last 3 vals passed in; false if misses sphere
}
...
// 0 10 -120 //origin
// 0 -- center of screen x axis
// 0 -- center of screen y axis
// -120 -- z close or far more neg; further away; less neg: closer
// Your initialization code goes here.
void PreInitializeMyStuff()
{
    //if(!the_scene.Load("model.txt"))
    if(!the_scene.Load("atom.txt"))
    {
        exit(1);
    }
